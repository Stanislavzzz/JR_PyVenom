...
Named volume хранится и управляется Docker.
...
## ▶️ Подключить named volume

```bash
docker run -v app_data:/app/data -d --name message-container message-app:1.1
```

Формат:

```text
имя_volume:путь_в_контейнере
```
...
## 🔄 Volume сохраняется после удаления контейнера

```bash
docker rm -f message-container
```

Создаём новый контейнер с тем же volume:

```bash
docker run -v app_data:/app/data -d --name message-container-2 message-app:1.1
```

Проверяем данные:

```bash
docker exec message-container-2 cat /app/data/messages.txt
```

Данные сохраняются, потому что принадлежат volume, а не контейнеру.
...
```bash
docker run -v app_data:/app/data -d --name container-one message-app:1.1
docker run -v app_data:/app/data -d --name container-two message-app:1.1
```

Оба контейнера работают с одним хранилищем:
...
Используемый контейнером volume сначала необходимо отключить или удалить контейнер.
...
⚠️ Использовать осторожно: будут удалены неиспользуемые Docker volumes.
...
Bind mount
./data:/app/data
→ данные находятся в конкретной папке компьютера
→ удобно для кода и конфигурации

Named volume
app_data:/app/data
→ данные управляются Docker
→ удобно для БД и постоянных данных
```
...
Docker создаёт volume автоматически, но управлять им сложнее, потому что понятного имени мы не задаём.
...
Данные хранятся в оперативной памяти и исчезают после остановки/удаления контейнера.
...
```text
bridge → обычная сеть контейнеров
host   → сеть хоста
none   → контейнер без обычного сетевого доступа
```
...
По умолчанию создаётся сеть с драйвером:
...
## 🔗 Проверить связь контейнеров

```bash
docker exec container-one ping -c 3 container-two
```

В пользовательской Docker-сети контейнеры могут обращаться друг к другу **по имени**.
...
Не нужно вручную использовать IP контейнера.
...
указывает на **сам текущий контейнер**.

```text
container-one
└── localhost → container-one
```

Чтобы обратиться к соседнему контейнеру:
...
```bash
docker network connect app-network container-one
```

Отключить:

```bash
docker network disconnect app-network container-one
```

Перезапуск контейнера для этого не нужен.
...
Сначала удалить или отключить контейнеры:

```bash
docker rm -f container-one container-two
```
...
docker run -d --name container-one --network app-network alpine sleep 3000
docker run -d --name container-two --network app-network alpine sleep 3000

# Проверить сеть
docker network inspect app-network

# Проверить связь по имени
docker exec container-one ping -c 3 container-two
